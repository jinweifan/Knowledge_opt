...
| 严格准确率 | **80.0%** | 是否幻觉 + 类型都对 |
| 类型命中率 | **77.8%** | 已检出样本中类型一致比例 |

- **漏检 (FN=0)**：无
- **误报 (FP=0)**：无
- **类型识别错误（4）**：h01、h10、h19、h20（已检出有问题，但类型判错）
- 对应知识出现在 Top-K：18/20
...
| Case | 标注 | 预测 | 为何判错 |
| ---- | ---- | ---- | -------- |
| h01 | 政策编造 | 政策偏差 | 将编造政策误判为政策细节偏差 |
| h10 | 能力越界 | 信息编造 | 将能力越界误判为信息编造 |
| h19 | 优惠编造 | 政策编造 | 将优惠编造误判为政策编造 |
| h20 | 信息遗漏 | 信息编造 | 将信息遗漏误判为信息编造 |
